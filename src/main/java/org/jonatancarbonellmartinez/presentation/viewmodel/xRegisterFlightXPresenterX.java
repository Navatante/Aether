...
public class xRegisterFlightXPresenterX implements xPresenter, xDialogPresenter {
    ...
    public xRegisterFlightXPresenterX(xRegisterFlightXDialogXView registerFlightDialogView, Observer observer) {
    ...
    public void insertEntity() { // TODO quiza este llame a todos los metodos de insertX
        try {
            ...
            xDialogView.showMessage(view,"Vuelo añadido correctamente.");

        } catch (DatabaseException ex) {
            xDialogView.showError(view,"Error al añadir vuelo: ");
        } catch (Exception ex) {
    ...
    public void onSaveButtonClicked() {
        ...
        if (isFormValid()) {
            ...
            xRegisterFlightXDialogXView newView = new xRegisterFlightXDialogXView(view.getMainView(), location);
    ...
    private boolean isVueloCardValid() {
        boolean isValid = xDialogPresenter.validateDynamicComboBox(view, view.getHeloBox(),"Helicóptero") &&
                            xDialogPresenter.validateDynamicComboBox(view, view.getEventBox(),"Evento") &&
                            xDialogPresenter.isFieldCompleted(view, view.getTotalHoursField(),"Horas totales");
    ...
    private boolean arePilotsSelected() {
        ...
        boolean isPilot1Selected = xDialogPresenter.validateDynamicComboBox(view, view.getPilotCardPanel1().getCrewBox(), "HAC");
        boolean isPilot2Selected = xDialogPresenter.validateDynamicComboBox(view, view.getPilotCardPanel2().getCrewBox(), "H2P");

        // Validate all extra pilot boxes
        boolean areExtraPilotsSelected = view.getExtraPilotCardPanelDeque()
                .stream()
                .allMatch(panel -> xDialogPresenter.validateDynamicComboBox(view, panel.getCrewBox(), "Extra H2P"));
    ...
    private boolean areDvsSelected() {
        // Validate pilot boxes for primary and secondary pilots
        boolean isDv1Selected = xDialogPresenter.validateDynamicComboBox(view, view.getDvCardPanel1().getCrewBox(), "Primer DV");

        // Validate all extra dv boxes
        boolean areExtraDvsSelected = view.getExtraDvCardPanelDeque()
                .stream()
                .allMatch(panel -> xDialogPresenter.validateDynamicComboBox(view, panel.getCrewBox(), "Extra DV"));
